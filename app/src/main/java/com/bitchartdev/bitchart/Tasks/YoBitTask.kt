 ...
 */
class YoBitTask: AsyncTask<WeakReference<BitChartActivity>, Unit, WeakReference<BitChartActivity>>() {
    override fun doInBackground(vararg p: WeakReference<BitChartActivity>): WeakReference<BitChartActivity> {
        ...
        val response = TaskHelper.getResponse(activity.getString(R.string.yobit_link))
        if (response != null) {
            // TODO: Parse response
        } else {
            // TODO: Catch error
